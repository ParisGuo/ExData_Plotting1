...
globalReactivePower <- as.numeric(sub_powerDT$Global_reactive_power)
voltage <- as.numeric(sub_powerDT$Voltage)

png("plot4.png")
par(mfrow=c(2,2))

plot(datetime, activepower, type="l", xlab = "", ylab = "Global Active Power(kilowatts)")
plot(datetime, voltage, type="l", xlab = "detetime", ylab = "Voltage")
...
plot(datetime, globalReactivePower, type="l", xlab = "detetime", ylab = "Global_reactive_power")
dev.off()